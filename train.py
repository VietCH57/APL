...
def main(args):
    ...
    train_dataset = APLSupervisedDataset(args.train_csv, args.vocab_path)
    dev_dataset = APLSupervisedDataset(args.dev_csv, args.vocab_path)
    ...
    for epoch in range(args.epochs):
        ...
        print(f"Epoch {epoch+1} Complete. Average Training Loss: {epoch_loss:.4f}")
        
        # 6. Evaluate on Dev Set every N epochs
        if (epoch + 1) % args.eval_every_epochs == 0 or (epoch + 1) == args.epochs:
            model.eval()
            all_hyps, all_trans, all_canons = [], [], []
            
            with torch.no_grad():
                for batch in tqdm(dev_loader, desc=f"Evaluating at Epoch {epoch+1}"):
                    waveforms = batch['waveforms'].to(device)
                    linguistics = batch['linguistics'].to(device)
                    transcripts = batch['transcripts'].to(device)
                    target_lengths = batch['target_lengths']
            ...
            if metrics['MDD_F_measure'] > best_f1:
                ...
                print("=> Saved new best checkpoint successfully!")
        else:
